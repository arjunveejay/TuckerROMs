...
def plot_errors(errs, methods=None, xlabel=r"$r$", figsize=None):
    """2×2 error plot for Q and P fields with median + IQR band.
    ...
    errs    : dict-like (e.g. np.load(...))
              ...
              ``{m}_rom_P_train``, ``{m}_proj_P_train``, ``{m}_rom_P_test``,
              ``{m}_proj_P_test`` for each method ``m`` in *methods*.
    ...
    _subplot(ax[0, 0], r"$\boldsymbol{q}$ Training", "{m}_rom_train",   "{m}_proj_train")
    _subplot(ax[0, 1], r"$\boldsymbol{q}$ Testing",  "{m}_rom_test",    "{m}_proj_test")
    _subplot(ax[1, 0], r"$\boldsymbol{p}$ Training", "{m}_rom_P_train", "{m}_proj_P_train")
    _subplot(ax[1, 1], r"$\boldsymbol{p}$ Testing",  "{m}_rom_P_test",  "{m}_proj_P_test")
...
def eval_grid(fom, q_vec, N=80):
    """Evaluate a Wave FOM solution vector on an N×N regular grid."""
    gfu = ng.GridFunction(fom.W)
    gfu.vec.FV().NumPy()[:] = q_vec
...
def plot_contours(fom, Z_fom, Z_rbf, Z_mono, Z_lid,
    ...
    Parameters
    ----------
    fom       : WaveFEM2D instance (for domain length L)
